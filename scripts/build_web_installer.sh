...
#
# Produces docs/firmware/eAIS-x3-<version>.bin (a single image containing
...
PIO="${PIO:-$(command -v pio || echo "$HOME/.venvs/pio/bin/pio")}"
ESPTOOL="${ESPTOOL:-$(command -v esptool || echo "$HOME/.venvs/pio/bin/esptool")}"
...
DATE="$(date +%Y-%m-%d)"
OUT="docs/firmware/eAIS-x3-${VERSION}.bin"

echo "==> Merging flash image  (version ${VERSION})"
mkdir -p docs/firmware
rm -f docs/firmware/eAIS-x3-*.bin   # keep exactly one build in the repo
  ...
  "builds": [
    {
      "chipFamily": "ESP32-C3",
      "parts": [
        { "path": "firmware/eAIS-x3-${VERSION}.bin", "offset": 0 }
      ]
    }
  ]
}

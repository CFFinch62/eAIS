// DERIVED FROM eNMEA/src/net/NmeaSource.cpp - deliberately divergent.
//
...
void NmeaSource::handleByte(uint8_t c, SentenceCallback cb, void* ctx) {
  ++bytesReceived_;
  if (!lineReader_.feed(c, lineBuf)) return;
  if (cb != nullptr) cb(lineBuf, ctx);
}
...
void NmeaSource::pollUdp(SentenceCallback cb, void* ctx) {
  ...
  while (udp_.available() > 0) {
    handleByte(static_cast<uint8_t>(udp_.read()), cb, ctx);
  }
}

void NmeaSource::pollTcp(SentenceCallback cb, void* ctx) {
  if (settings_.host[0] == '\0') return;  // nothing to dial; begin() already said so

  if (tcp_.connected()) {
    bool gotBytes = false;
    while (tcp_.available() > 0) {
      handleByte(static_cast<uint8_t>(tcp_.read()), cb, ctx);
      gotBytes = true;
    }
    if (gotBytes) {
      lastRxMs_ = millis();
...
void NmeaSource::poll(SentenceCallback cb, void* ctx) {
  if (settings_.protocol == NmeaProfile::Protocol::UDP) {
    pollUdp(cb, ctx);
  } else {
    pollTcp(cb, ctx);

...
// NmeaSentence::parse() splits fields in place - it writes NULs into its
// argument - so each call needs its own mutable copy. Passing a string literal
// would be undefined behaviour; this is the only sanctioned way to parse here.
AisDecoder::Result feed(AisDecoder& decoder, const char* sentence, AisTargetTable& targets, OwnShip& own,
                        unsigned long nowMs);
